...
-- read it. The name and the metadata are plain text, so the
-- server can list and search by them. "version" grows on every
-- update (last write wins). "deleted" is a soft-delete flag.
